...
type Props = { onDetected: (text: string) => void; onManual?: () => void };
...
export default function Html5BarcodeScanner({ onDetected, onManual }: Props) {
  ...
  // --- NEW: bentuk qrbox & device id terakhir (untuk restart saat toggle) ---
  ...
  //   // simpan device id terakhir untuk restart otomatis
  ...
  //     },
  //     // properti ini tersedia di versi baru
  //     formatsToSupport: [
  ...
  //       deviceId,
  //       config,
  //       // Tidak langsung panggil onDetected: tahan dulu di pending
  //       (text) => {
  ...
  //       () => {}
  //     );
  //     await setupControlsFromVideo();

  //     // --- set default zoom 3.5x jika kamera support zoom ---
  //     const c = (track as any)?.getCapabilities?.() || caps || {};
  //     const s: any = (track as any)?.getSettings?.() || {};
  //     if (c?.zoom) {
  //       const minZoom = c.zoom.min ?? 1;
  //       const maxZoom = c.zoom.max ?? 5;
  //       const targetZoom = Math.min(Math.max(DEFAULT_ZOOM, minZoom), maxZoom);

  //       // Hanya apply kalau current bukan target (menghindari flicker)
  //       const cur =
  //         typeof s?.zoom === "number" ? (s.zoom as number) : minZoom ?? 1;
  //       if (Math.abs(cur - targetZoom) > 1e-3) {
  //         await safeApply(track, { advanced: [{ zoom: targetZoom }] });
  //       }
  ...
  async function startWith(deviceId: string) {
    ...
    try {
      await qr.start(
        ...
        config,
        (text) => {
          if (modalScanRef.current == false) setPending(text);
        },
        () => {}
      );

      // ---- AMBIL TRACK LANGSUNG, JANGAN ANDALKAN STATE ----
      // beri 1–2 frame supaya <video> & srcObject terpasang
      await sleep(30);
      const video = document.querySelector<HTMLVideoElement>(
        `#${containerId} video`
      );
    ...
    <>
      <div className="w-full text-center">
        ...
        <div style={{ display: "inline-block", position: "relative" }}>
          ...
          <div
            id={containerId}
            className="md:w-[560px] w-[100vw] h-full bg-black rounded-[8px] overflow-hidden"
          />
        ...
        >
          ...
          >
            ...
            <button onClick={setAF_Continuous} disabled={!afContinuous} className="px-3 py-1 rounded border border-gray-300">
              Auto Focus
            </button>
            <button onClick={setAF_SingleShot} disabled={!afSingleShot} className="px-3 py-1 rounded border border-gray-300">
              Single-shot AF
            </button>
            */}
      ...
      {isModalSuccessScan && (
        <ModalTemplate
          closeModal={() => setIsModalSuccessScan(false)}
